...
| `REPORT_LANGUAGE` | 报告输出语言：`zh`(默认中文) / `en`(英文)；会同步影响 Prompt、模板与 Web 报告页固定文案 | 可选 |
...
| `LONGBRIDGE_PUSH_CANDLESTICK_MODE` | 长桥 K 线订阅模式：`realtime` 或 `confirmed`（默认 `realtime`） | 可选 |
...
- 混合市场标的时，每只股票只在其市场开市日分析，休市股票当日跳过
- 全部相关市场均为非交易日时，整体跳过执行（不启动 pipeline）
...
WebUI 与 FastAPI API 服务共用同一服务进程，启动后可在浏览器中完成配置管理、手动分析、任务进度查看、历史报告和智能导入等操作。认证、云服务器访问和 API 调用细节见下方说明。
