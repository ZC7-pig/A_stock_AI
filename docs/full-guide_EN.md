...
│   ├── report_service.py # Report generation and local saving
│   └── ...
...
#### Report and Execution Configuration

| Secret Name | Description | Required |
|------------|------|:----:|
| `REPORT_TYPE` | Report type: `simple` (concise), `full` (complete), `brief` (3-5 sentences), Docker recommended: `full` | Optional |
| `REPORT_LANGUAGE` | Report output language: `zh` (default Chinese) / `en` (English); also updates prompt instructions, templates, and fixed copy in the Web report view | Optional |
...
The WebUI and FastAPI API share the same service process. After startup, use the browser workspace for configuration management, manual analysis, task progress, historical reports, and smart import. Authentication, cloud-server access, and API usage details are covered below.
...
- Generated reports are available in the history view after analysis completes
