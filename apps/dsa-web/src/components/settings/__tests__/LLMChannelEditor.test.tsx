...
describe('LLMChannelEditor', () => {
  ...
  it('ignores legacy YAML routing fields and keeps runtime controls visible', () => {
    render(
      <LLMChannelEditor
        items={[
          { key: 'LLM_CHANNELS', value: 'openai' },
    ...
    expect(screen.getByText('运行时参数')).toBeInTheDocument();
    expect(screen.getByLabelText('主模型')).toBeInTheDocument();
    expect(screen.queryByText(/高级模型路由 YAML/i)).not.toBeInTheDocument();
