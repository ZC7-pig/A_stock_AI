    ...
    <div className="fixed inset-0 overflow-hidden" style={{ zIndex }} role="presentation">
      ...
      <div className={cn('absolute inset-y-0 flex w-full', sidePositionClass, width)}>
        <div
          ...
          className={cn(
            'relative flex w-full flex-col bg-[var(--v4-surface)]',
            borderClass,
            side === 'right' ? 'border-[var(--v4-line-strong)]' : 'border-[var(--v4-line-strong)] shadow-2xl',
            side === 'left' ? 'animate-slide-in-left' : 'animate-slide-in-right'
          )}
        >
          <div className="flex items-center justify-between border-b border-[var(--v4-line)] px-6 py-4">
            {title ? (
              <div>
            ...
            <button
              type="button"
              onClick={onClose}
              className="inline-flex h-10 w-10 items-center justify-center rounded-[var(--v4-radius-sm)] border border-[var(--v4-line)] bg-[var(--v4-surface-soft)] text-secondary-text transition-colors hover:bg-[var(--v4-theme-soft)] hover:text-foreground"
              aria-label="关闭抽屉"
            >
