...
const truncateQuickRead = (value: string, fallback: string): string => {
...
const quickReadToneClass: Record<QuickReadItem['tone'], string> = {
  primary: 'home-report-quick-item-primary',
  success: 'home-report-quick-item-success',
  warning: 'home-report-quick-item-warning',
};
...
export const ReportOverview: React.FC<ReportOverviewProps> = ({
  ...
  const quickReadItems: QuickReadItem[] = [
    {
      label: text.sentimentScore,
      value: `${summary.sentimentScore} / ${getSentimentLabel(summary.sentimentScore, reportLanguage)}`,
      tone: 'primary',
    },
    {
      label: text.actionAdvice,
      value: truncateQuickRead(summary.operationAdvice, text.noAdvice),
      tone: 'success',
    },
    {
      label: text.trendPrediction,
      value: truncateQuickRead(summary.trendPrediction, text.noPrediction),
      tone: 'warning',
    },
  ];

  return (
    <section aria-label={text.reportOverview} className="space-y-5">
          ...
          <Card variant="gradient" padding="md" className="home-report-hero">
            <div className="flex items-start justify-between mb-5">
              <div className="flex-1">
                <span className="label-uppercase">{text.researchBrief}</span>
            ...
            <div className="mt-5">
              <span className="label-uppercase">{text.quickJudgement}</span>
              <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-3">
                {quickReadItems.map((item) => (
                  <div
                    key={item.label}
                    className={`home-report-quick-item ${quickReadToneClass[item.tone]}`}
                  >
                    <span className="text-[11px] font-medium text-muted-text">{item.label}</span>
