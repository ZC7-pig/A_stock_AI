...
function setSessionStorageValue(sessionId: string) {
  ...
  try {
    ...
    // Storage can be unavailable in hardened browser/test environments.
  }
}
...
const getInitialSessionId = (): string => getSessionStorageValue() || generateUUID();
...
export const useAgentChatStore = create<AgentChatState & AgentChatActions>((set, get) => ({
  ...
  loadInitialSession: async () => {
    ...
    try {
      ...
      const savedId = getSessionStorageValue();
      if (savedId) {
        ...
        if (sessionExists) {
          ...
          set({ sessionId: newId });
          setSessionStorageValue(newId);
        }
      } else {
        setSessionStorageValue(get().sessionId);
      }
    } catch {
      // Ignore
    } finally {
  ...
  switchSession: async (targetSessionId) => {
    ...
    setSessionStorageValue(targetSessionId);
