...
function toSnakeTestDataSourcePayload(payload: TestDataSourceConnectionRequest): Record<string, unknown> {
  return {
    key: payload.key,
    value: payload.value ?? '',
    timeout_seconds: payload.timeoutSeconds ?? 10,
  };
}
...
export const systemConfigApi = {
  ...
  async testDataSourceConnection(
    ...
    return toCamelCase<TestDataSourceConnectionResponse>(response.data);
  },
