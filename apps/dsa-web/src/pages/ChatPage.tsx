...
const ChatPage: React.FC = () => {
  ...
  const [followUpContextLabel, setFollowUpContextLabel] = useState('未关联报告上下文');
  ...
  const selectedSkillInfo = skills.find((skill) => skill.id === selectedSkill);
  const selectedSkillName = selectedSkillInfo?.name || (selectedSkill ? selectedSkill : '通用分析');

  const handleStartNewChat = useCallback(() => {
    followUpContextRef.current = null;
    setFollowUpContextLabel('未关联报告上下文');
  ...
  useEffect(() => {
    ...
    window.requestAnimationFrame(() => {
      ...
      if (recordId !== undefined) {
        ...
        setFollowUpContextLabel('正在加载历史报告');
      } else {
        setFollowUpContextLabel('已关联股票上下文');
      }
      void resolveChatFollowUpContext({
        ...
        setFollowUpContextLabel(context.previous_analysis_summary ? '已关联历史报告' : '已关联股票上下文');
      }).finally(() => {
        if (isMountedRef.current && followUpHydrationTokenRef.current === hydrationToken) {
  ...
  return (
    <div
      ...
      className="v4-page v4-chat-page flex h-screen w-full min-w-0 gap-4 overflow-hidden p-4"
    >
      {/* Desktop sidebar */}
      <aside
        aria-label="对话历史"
        className="v4-panel hidden h-full w-[298px] flex-shrink-0 flex-col overflow-hidden md:flex"
      >
        {sidebarContent}
      </aside>
      ...
      {sidebarOpen && (
        ...
        >
          ...
          <div
            className="v4-panel absolute left-0 top-0 bottom-0 flex w-72 flex-col overflow-hidden !rounded-none !rounded-r-xl shadow-2xl"
      ...
      {/* Main chat area */}
      <main
        aria-label="问股 Agent 工作区"
        className="flex h-full min-w-0 flex-1 flex-col overflow-hidden"
      >
        <header className="v4-chat-header mb-4 flex-shrink-0 space-y-3">
          ...
          <p className="text-secondary-text text-sm">
            围绕个股、历史报告和策略技能追问，沉淀可复盘的研究结论。
          </p>
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
            <div className="chat-research-status">
              <span>研究模式</span>
              <strong>{selectedSkillName}</strong>
            </div>
            <div className="chat-research-status">
              <span>报告上下文</span>
              <strong>{followUpContextLabel}</strong>
            </div>
            <div className="chat-research-status">
              <span>输出重点</span>
              <strong>结论 · 风险 · 点位</strong>
            </div>
          </div>
        </header>

        <div className="v4-chat-main-panel relative z-10 flex min-h-0 flex-1 flex-col overflow-hidden">
          {/* Messages */}
          <ScrollArea
          ...
          {/* Input area */}
          <div className="relative z-20 border-t border-[var(--v4-line)] bg-white/80 p-4 md:p-6">
            ...
            {skills.length > 0 && (
              <div className="chat-skill-select-row">
                <label htmlFor="chat-skill-select" className="text-xs font-medium uppercase tracking-wider text-muted-text">
                  研究视角
                </label>
                <select
                  ...
                  className="input-surface input-focus-glow min-h-[36px] rounded-lg border bg-transparent px-3 py-1.5 text-sm text-foreground focus:outline-none"
                >
                ...
                {selectedSkillInfo?.description ? (
                  <span className="min-w-0 truncate text-xs text-muted-text">
                    {selectedSkillInfo.description}
                  </span>
                ) : null}
              </div>
