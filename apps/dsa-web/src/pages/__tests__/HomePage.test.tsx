...
describe('HomePage', () => {
  ...
  it('renders the dashboard workspace and auto-loads the first report', async () => {
    ...
    expect(screen.getByRole('banner', { name: '股票分析输入' })).toBeInTheDocument();
    expect(screen.getByRole('complementary', { name: '研究侧栏' })).toBeInTheDocument();
    ...
    expect((await screen.findAllByText('趋势维持强势')).length).toBeGreaterThan(0);
    expect(
  ...
  it('shows the empty report workspace when history is empty', async () => {
    ...
    expect(await screen.findByText('研究准备区')).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: '开始单股分析', level: 2 })).toBeInTheDocument();
    expect(screen.getByText('顶部搜索框输入代码或名称，然后点击“开始分析”')).toBeInTheDocument();
    expect(screen.getByText('暂无报告')).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: '暂无报告', level: 3 })).toBeInTheDocument();
    expect(screen.getByText('当前没有选中的历史报告，也没有生成中的结果。')).toBeInTheDocument();
  ...
  it('navigates to chat with report context when asking a follow-up question', async () => {
    ...
    const followUpButton = await screen.findByRole('button', { name: '展开追问' });
    fireEvent.click(followUpButton);

    expect(await screen.findByText('基于当前报告追问')).toBeInTheDocument();
    fireEvent.click(screen.getByRole('button', { name: '进入问股 Agent' }));
  ...
  it('opens and closes the mobile history drawer without changing dashboard styles', async () => {
    ...
    expect(container.querySelector('.v4-panel')).toBeTruthy();
