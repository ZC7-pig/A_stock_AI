...
class ConfigEnvCompatibilityTestCase(unittest.TestCase):
    ...
    @patch("src.config.setup_env")
    @patch.object(Config, "_parse_litellm_yaml", return_value=[])
    def test_notification_env_values_are_ignored(
        self,
        _mock_parse_yaml,
        _mock_setup_env,
    ) -> None:
        env = {
            ...
            "CUSTOM_WEBHOOK_URLS": "https://example.com/custom",
            "SINGLE_STOCK_NOTIFY": "true",
            "REPORT_SUMMARY_ONLY": "true",
            "MERGE_EMAIL_NOTIFICATION": "true",
            "MARKDOWN_TO_IMAGE_CHANNELS": "wechat,email",
        }

        with patch.dict(os.environ, env, clear=True):
            config = Config._load_from_env()

        self.assertNotIn("wechat_webhook_url", config.__dict__)
        self.assertNotIn("feishu_webhook_url", config.__dict__)
        self.assertNotIn("email_sender", config.__dict__)
        self.assertNotIn("pushplus_token", config.__dict__)
        self.assertNotIn("custom_webhook_urls", config.__dict__)
        self.assertNotIn("single_stock_notify", config.__dict__)
        self.assertNotIn("report_summary_only", config.__dict__)
        self.assertNotIn("merge_email_notification", config.__dict__)
        self.assertNotIn("markdown_to_image_channels", config.__dict__)
