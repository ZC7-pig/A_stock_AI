...
class SystemConfigServiceTestCase(unittest.TestCase):
    ...
    def test_get_config_returns_raw_sensitive_values(self) -> None:
        ...
        self.assertNotIn("GEMINI_API_KEY", items)
        self.assertIn("LLM_CHANNELS", items)
        self.assertIn("LLM_GEMINI_API_KEY", items)
        self.assertEqual(items["LLM_CHANNELS"]["value"], "gemini")
        self.assertEqual(items["LLM_GEMINI_API_KEY"]["value"], "secret-key-value")
        self.assertFalse(items["LLM_GEMINI_API_KEY"]["is_masked"])
        self.assertTrue(items["LLM_GEMINI_API_KEY"]["raw_value_exists"])
    ...
    def test_removed_notification_keys_are_hidden_from_config_payload(self) -> None:
        self._rewrite_env(
            "STOCK_LIST=600519,000001",
        ...
        self.assertNotIn("WECHAT_WEBHOOK_URL", item_keys)
        self.assertNotIn("FEISHU_WEBHOOK_URL", item_keys)
        self.assertNotIn("FEISHU_APP_ID", item_keys)
        self.assertNotIn("FEISHU_APP_SECRET", item_keys)
        self.assertNotIn("EMAIL_SENDER", item_keys)
        self.assertNotIn("PUSHPLUS_TOKEN", item_keys)
    ...
    @patch("src.services.system_config_service.TavilySearchProvider")
    def test_test_data_source_connection_uses_first_configured_search_key(self, mock_provider_cls) -> None:
        provider = mock_provider_cls.return_value
        provider.search.return_value = SimpleNamespace(
            success=True,
            provider="Tavily",
            error_message=None,
            search_time=0.123,
            results=[SimpleNamespace(title="ok")],
        )

        payload = self.service.test_data_source_connection(
            key="TAVILY_API_KEYS",
        ...
        self.assertEqual(payload["tested_key"], "TAVILY_API_KEYS")
        self.assertEqual(payload["latency_ms"], 123)
        mock_provider_cls.assert_called_once_with(["key-one"])
        provider.search.assert_called_once_with("贵州茅台 股票", max_results=1, days=7)

    def test_test_data_source_connection_rejects_unsupported_field(self) -> None:
        payload = self.service.test_data_source_connection(
        ...
        self.assertEqual(payload["tested_key"], "REALTIME_SOURCE_PRIORITY")
