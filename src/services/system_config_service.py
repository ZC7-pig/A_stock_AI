...
class SystemConfigService:
    ...
    _LEGACY_AI_DISPLAY_KEYS: Set[str] = {
        "AIHUBMIX_KEY",
        "DEEPSEEK_API_KEY",
        "DEEPSEEK_API_KEYS",
        "GEMINI_API_KEY",
    ...
    _REMOVED_NOTIFICATION_DISPLAY_KEYS: Set[str] = {
        "WECHAT_WEBHOOK_URL",
        "FEISHU_WEBHOOK_URL",
    ...
    @staticmethod
    def _append_unique_csv(existing: str, values: Sequence[str]) -> str:
        result: List[str] = []
        seen: Set[str] = set()
        for raw_value in [existing, *values]:
    ...
    def _add_legacy_ai_channel(
        ...
        *,
        ...
        """Expose old provider env vars as unified LLM channel fields for migration."""
        key_value = ""
        key_target = f"LLM_{channel.upper()}_API_KEY"
        for candidate in key_candidates:
            value = (raw_upper.get(candidate) or "").strip()
            if not value:
                continue
            key_value = value
            key_target = f"LLM_{channel.upper()}_API_KEYS" if candidate.endswith("_KEYS") else key_target
            break

        model_values = [
            (raw_upper.get(candidate) or "").strip()
            for candidate in model_candidates
            if (raw_upper.get(candidate) or "").strip()
        ]
        if not key_value and not model_values:
            return

        prefix = f"LLM_{channel.upper()}"
        existing_channel_keys = [
            key for key in raw_upper
            if key.startswith(f"{prefix}_") or key == "LLM_CHANNELS"
        ]
        if any(key.startswith(f"{prefix}_") for key in existing_channel_keys):
            return

        display_map["LLM_CHANNELS"] = cls._append_unique_csv(display_map.get("LLM_CHANNELS", ""), [channel])
        display_map[f"{prefix}_PROTOCOL"] = protocol
        if base_url:
            display_map[f"{prefix}_BASE_URL"] = base_url
        if key_value:
            display_map[key_target] = key_value
        if model_values:
            display_map[f"{prefix}_MODELS"] = cls._append_unique_csv("", model_values)
    ...
    def _build_display_config_map(cls, raw_config_map: Dict[str, str]) -> Dict[str, str]:
        ...
        for key, value in raw_upper.items():
            if (
                key in aliased_keys
                or key in cls._LEGACY_AI_DISPLAY_KEYS
                or key in cls._REMOVED_NOTIFICATION_DISPLAY_KEYS
            ):
                continue
        ...
        cls._add_legacy_ai_channel(
            display_map,
            raw_upper,
            channel="gemini",
            protocol="gemini",
            key_candidates=("GEMINI_API_KEYS", "GEMINI_API_KEY"),
            model_candidates=("GEMINI_MODEL", "GEMINI_MODEL_FALLBACK"),
        )
        cls._add_legacy_ai_channel(
            display_map,
            raw_upper,
            channel="openai",
            protocol="openai",
            key_candidates=("OPENAI_API_KEYS", "OPENAI_API_KEY"),
            model_candidates=("OPENAI_MODEL", "OPENAI_VISION_MODEL", "VISION_MODEL"),
            base_url=(raw_upper.get("OPENAI_BASE_URL") or "").strip() or "https://api.openai.com/v1",
        )
        cls._add_legacy_ai_channel(
            display_map,
            raw_upper,
            channel="deepseek",
            protocol="openai",
            key_candidates=("DEEPSEEK_API_KEYS", "DEEPSEEK_API_KEY"),
            model_candidates=(),
        ...
        cls._add_legacy_ai_channel(
            ...
            channel="anthropic",
            protocol="anthropic",
            key_candidates=("ANTHROPIC_API_KEYS", "ANTHROPIC_API_KEY"),
            model_candidates=("ANTHROPIC_MODEL",),
        )
        cls._add_legacy_ai_channel(
            display_map,
            raw_upper,
    ...
    @staticmethod
    def _first_config_value(value: str) -> str:
    ...
    def test_data_source_connection(
        ...
        *,
        ...
        """Run a lightweight connectivity test for one data-source setting."""
        tested_key = key.strip().upper()
        raw_value = str(value or "").strip()
        if not raw_value:
            return self._connection_result(
                success=False,
                message="Data source value is empty",
                error="请先填写配置值再测试连接",
                code="empty_value",
                tested_key=tested_key,
            )
        ...
        search_provider_map = {
            "ANSPIRE_API_KEYS": ("Anspire", AnspireSearchProvider),
            "BOCHA_API_KEYS": ("Bocha", BochaSearchProvider),
            "BRAVE_API_KEYS": ("Brave", BraveSearchProvider),
            "MINIMAX_API_KEYS": ("MiniMax", MiniMaxSearchProvider),
            "SERPAPI_API_KEYS": ("SerpAPI", SerpAPISearchProvider),
            "TAVILY_API_KEYS": ("Tavily", TavilySearchProvider),
        }
        if tested_key in search_provider_map:
            provider_label, provider_cls = search_provider_map[tested_key]
            api_key = self._first_config_value(raw_value)
            if not api_key:
                return self._connection_result(
                    success=False,
                    message="Data source value is empty",
                    error="未找到可测试的 API Key",
                    code="empty_value",
                    tested_key=tested_key,
            ...
            try:
                ...
                response = provider.search("贵州茅台 股票", max_results=1, days=7)
            except Exception as exc:
                latency_ms = int((time.perf_counter() - started_at) * 1000)
                logger.warning("Data source connection test failed for %s: %s", tested_key, exc)
                return self._connection_result(
                    success=False,
            ...
            latency_ms = int((getattr(response, "search_time", 0.0) or (time.perf_counter() - started_at)) * 1000)
            success = bool(getattr(response, "success", False))
            return self._connection_result(
                success=success,
                message="Data source connection succeeded" if success else "Data source connection failed",
                error=None if success else getattr(response, "error_message", None),
                code="ok" if success else "request_failed",
                tested_key=tested_key,
                provider=getattr(response, "provider", None) or provider_label,
                latency_ms=latency_ms,
            )

        if tested_key == "SEARXNG_BASE_URLS":
            base_url = self._first_config_value(raw_value)
            started_at = time.perf_counter()
            try:
                response = SearXNGSearchProvider([base_url], use_public_instances=False).search(
                    "贵州茅台 股票",
                    max_results=1,
                    days=7,
                )
        ...
        if tested_key == "TUSHARE_TOKEN":
            token = self._first_config_value(raw_value)
            started_at = time.perf_counter()
            try:
                ...
                return self._connection_result(
                    success=success,
                    message="Data source connection succeeded" if success else "Data source connection failed",
                    error=None if success else str(payload.get("msg") or payload),
                    code="ok" if success else "request_failed",
                    tested_key=tested_key,
                    provider="Tushare",
                    latency_ms=latency_ms,
        ...
        if tested_key == "TICKFLOW_API_KEY":
            started_at = time.perf_counter()
            try:
                from data_provider.tickflow_fetcher import TickFlowFetcher

                fetcher = TickFlowFetcher(api_key=self._first_config_value(raw_value), timeout=max(5.0, float(timeout_seconds)))
                try:
                ...
                return self._connection_result(
                    success=success,
                    message="Data source connection succeeded" if success else "Data source connection failed",
                    error=None if success else "TickFlow 未返回主要指数行情",
                    code="ok" if success else "request_failed",
                    tested_key=tested_key,
                    provider="TickFlow",
                    latency_ms=latency_ms,
        ...
        return self._connection_result(
            success=False,
            message="Data source connection test is not supported for this field",
            error="该配置项暂不支持连通性测试",
            code="unsupported",
            tested_key=tested_key,
        )
