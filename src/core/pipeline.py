...
class StockAnalysisPipeline:
    ...
    def __init__(
        ...
        self.report_service = ReportService(source_message=source_message)
    ...
    def _save_local_report(
        ...
        try:
            ...
            filepath = self.report_service.save_report_to_file(report)
    ...
    def _generate_aggregate_report(
        ...
        """Generate aggregate report with backward-compatible report-service fallback."""
        generator = getattr(self.report_service, "generate_aggregate_report", None)
        if callable(generator):
            return generator(results, report_type)
        if report_type == ReportType.BRIEF and hasattr(self.report_service, "generate_brief_report"):
            return self.report_service.generate_brief_report(results)
        return self.report_service.generate_dashboard_report(results)
