...
3. **Skill performance hooks** — currently return neutral weights until
   a focused performance store is introduced.

Storage uses the existing SQLAlchemy analysis-history layer rather than
...
class AgentMemory:
    ...
    def get_skill_performance(self, skill_id: str) -> Dict[str, Any]:
        """Return neutral performance until a focused performance store exists."""
        return {"available": False, "skill_id": skill_id}
    ...
    def compute_skill_weights(
        ...
        """
        if not self.enabled or not use_performance:
    ...
    def compute_strategy_weights(
        ...
        """Compatibility wrapper for legacy strategy-based callers."""
        return self.compute_skill_weights(strategy_ids, use_performance=use_performance)
    ...
    def _get_accuracy_stats(
        ...
        """Return neutral accuracy until a focused outcome store exists."""
