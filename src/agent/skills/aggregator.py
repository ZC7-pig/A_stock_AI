...
class SkillAggregator:
    ...
    def aggregate(
        self,
        ctx: AgentContext,
        min_samples: int = _MIN_PERFORMANCE_SAMPLES,
        ...
        perf_weights = (
            memory.compute_skill_weights(
                skill_ids,
                use_performance=self._use_performance_autoweight(),
            )
            if memory.enabled
            else {}
        )
    ...
    def _compute_weight(
        ...
        return base_weight * self._performance_factor(opinion.agent_name, min_samples)

    @staticmethod
    def _performance_factor(agent_name: str, min_samples: int) -> float:
        if not SkillAggregator._use_performance_autoweight():
            return 1.0
        ...
        try:
            ...
            if perf.get("sufficient_samples") and perf.get("total_evaluations", 0) >= min_samples:
                return 0.5 + perf.get("win_rate", 0.5)
        except Exception:
            logger.debug("Failed to compute performance factor for %s", agent_name, exc_info=True)
        return 1.0

    @staticmethod
    def _use_performance_autoweight() -> bool:
        try:
            from src.config import get_config

            config = get_config()
            return getattr(config, "agent_skill_autoweight", False)
        except Exception:
            logger.debug("Failed to get skill autoweight config, defaulting to False", exc_info=True)
            return False
