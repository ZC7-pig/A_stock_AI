...
Monitors a set of stocks for threshold events and triggers callbacks
when conditions are met. Designed to run as a background task
(e.g. via ``--schedule`` or a dedicated loop).
...
class TriggeredAlert:
    """An alert that was triggered and ready for downstream handling."""
    rule: AlertRule
...
class EventMonitor:
    ...
    and can be handled by registered callbacks.
    """
...
def build_event_monitor_from_config(config=None) -> Optional[EventMonitor]:
    """Build an EventMonitor from runtime config."""
    if config is None:
